...
Future<bool> agregarUsuario(Usuario usuario) async {
  try {
    ...
    Text('Error al agregar usuario: $e');
...
Future<void> agregarPizzaFavorita(String userUid, String pizzaUid) async {
  try {
    ...
    Text('Error al agregar pizza a favoritos: $e');
    rethrow;
  }
}
    ...
    if (usuarioDoc.exists) {
      ...
      if (userData != null && userData.containsKey('favoritePizzas')) {
        ...
        if (favoritasData is List) {
          ...
          const Text('El campo favoritePizzas no es una lista válida.');
        }
      } else {
        const Text('El usuario no tiene pizzas favoritas.');
      }
    } else {
      Text('El usuario con UID $uidUsuario no existe.');
    }

    return favoritas;
  } catch (e) {
    Text('Error al obtener las pizzas favoritas: $e');
    return [];
  }
}
